...
static const BG95_at_LUT_t ATCMD_BG95_LUT[] =
    {
        ...
        {CMD_AT, "", BG95_AT_TIMEOUT, fCmdBuild_NoParams, NULL}, /*Ultimo parametro en realidad es fRspAnalyze_None: chequear si me sirven*/
        ...
        {CMD_AT_NO_CARRIER, "NO CARRIER", BG95_DEFAULT_TIMEOUT, fCmdBuild_NoParams, NULL},
        {CMD_AT_ERROR, "ERROR", BG95_DEFAULT_TIMEOUT, fCmdBuild_NoParams, NULL},
        {CMD_AT_NO_DIALTONE, "NO DIALTONE", BG95_DEFAULT_TIMEOUT, fCmdBuild_NoParams, NULL},
        {CMD_AT_BUSY, "BUSY", BG95_DEFAULT_TIMEOUT, fCmdBuild_NoParams, NULL},
        {CMD_AT_NO_ANSWER, "NO ANSWER", BG95_DEFAULT_TIMEOUT, fCmdBuild_NoParams, NULL},
        {CMD_AT_CME_ERROR, "+CME ERROR", BG95_DEFAULT_TIMEOUT, fCmdBuild_NoParams, NULL},
        {CMD_AT_CMS_ERROR, "+CMS ERROR", BG95_DEFAULT_TIMEOUT, fCmdBuild_NoParams, NULL},
...
#define SIZE_ATCMD_BG95_LUT ((uint16_t)(sizeof(ATCMD_BG95_LUT) / sizeof(BG95_at_LUT_t)))
...
bool ATCore_send_cmd(atcmd_desc_t *cmd){
  ...
  ATCMD_BG95_LUT[cmd->id].build(command, ATCMD_BG95_LUT[cmd->id].cmd_string, cmd);
...
bool ATCore_is_response_ready(){
  return AT_DRV->is_response_ready(AT_DEVICE);
}

char* ATCore_get_last_response(){

  return AT_DRV->get_response(AT_DEVICE);
}
